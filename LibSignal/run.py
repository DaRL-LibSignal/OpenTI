...
parser.add_argument('-tt','--time', type=str, help='file time')
...
parser.add_argument('-a', '--agent', type=str, default="dqn", choices=['dqn', 'frap', 'presslight', 'mplight', 'fixedtime', 'sotl'], help="agent type of agents in RL environment")
parser.add_argument('-w', '--world', type=str, default="cityflow", choices=['cityflow','sumo'], help="simulator type")
parser.add_argument('-n', '--network', type=str, default="cityflow1x1", help="network name") # cityflow1x1, sumo1x1
parser.add_argument('-d', '--dataset', type=str, default='onfly', help='type of dataset in training process')
# python run.py -a dqn -w cityflow -n cityflow_cologne3
...
class Runner:
    ...
    def run(self):
        timestamp = args.time
        logger = setup_logging(logging_level,timestamp)
