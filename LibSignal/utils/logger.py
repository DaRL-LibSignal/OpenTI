...
#guanru's modification
class LoggerPathInterface:
    def __init__(self, config):
        self.param = config['logger']['path']
        Registry.mapping['logger_mapping']['path'] = self

    @property
    def path(self):
        return self.param

def setup_logging(level,timestamp):
    ...
    if not root.hasHandlers():
        ...
        handler_file = logging.FileHandler(os.path.join(
                ...
                #f"{datetime.now().strftime('%Y_%m_%d-%H_%M')}_BRF.log"), mode='w'
                f"{timestamp}_BRF.log"), mode='w'
            )
